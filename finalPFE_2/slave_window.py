...
WINDOW_LEDS = {
    "DR": [28, 27, 26, 25],           # Driver Window - LED 12–15
    "PS": [12, 13, 14, 15],           # Passenger Window - LED 28–25
    "DRS": [37, 31, 30, 29],            # Rear Driver Window - LED 3, 9–11
    "PRS": [24, 9, 10, 11]          # Rear Passenger Window - LED 16, 31, 30, 29
}
...
class CANWindowSlave:
    def __init__(self):
        ...
        self.LEDs_status = [0] * 80  # 40-bit shift register status
        self.lock = threading.Lock()
        self.led_update_lock = threading.Lock()  # New lock for LED updates
    ...
    def shift_out(self, data):
        ...
        with self.led_update_lock:  # Protect shift register updates
            for state in reversed(data):  # <-- REVERSED!
    ...
    def update_window_leds(self, window, new_level):
        ...
        # Animate LED changes with thread protection
