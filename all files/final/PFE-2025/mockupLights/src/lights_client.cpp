...
bool LightsClient::init() {
    if (!app_->init()) {
        std::cerr << get_current_timestamp() << " CLIENT: Failed to initialize vsomeip application!" << std::endl;
        return false;
    }
...
std::string LightsClient::get_current_timestamp() {
...
void LightsClient::on_message(const std::shared_ptr<vsomeip::message> &_response) {
    std::cout << get_current_timestamp() << " " << message_to_string(_response) << std::endl;
...
void LightsClient::turn_on_low_beam_headlights(const std::string &state) {
    std::string message = get_current_timestamp() + " Low Beam Headlights | " + state;
    std::cout << message << std::endl;
}

void LightsClient::turn_on_hazard_lights(const std::string &state) {
    std::string message = get_current_timestamp() + " Hazard Lights | " + state;
    std::cout << message << std::endl;
}

void LightsClient::turn_on_right_turn_signal(const std::string &state) {
    std::string message = get_current_timestamp() + " Right Turn Signal | " + state;
    std::cout << message << std::endl;
}

void LightsClient::turn_on_left_turn_signal(const std::string &state) {
    std::string message = get_current_timestamp() + " Left Turn Signal | " + state;
    std::cout << message << std::endl;
}

void LightsClient::turn_on_high_beam_headlights(const std::string &state) {
    std::string message = get_current_timestamp() + " High Beam Headlights | " + state;
    std::cout << message << std::endl;
}

void LightsClient::turn_on_parking_left_signal(const std::string &state) {
    std::string message = get_current_timestamp() + " Parking left Signal | " + state;
    std::cout << message << std::endl;
}

void LightsClient::turn_on_parking_right_signal(const std::string &state) {
    std::string message = get_current_timestamp() + " Parking right Signal | " + state;
    std::cout << message << std::endl;
}
