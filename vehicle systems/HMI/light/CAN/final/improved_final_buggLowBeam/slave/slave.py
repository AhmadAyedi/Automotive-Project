...
class CANLightSlave:
    ...
    def control_light(self, light, status_code, mode_code):
        """Control the light based on status and mode."""
        ...
        if mode_code not in [0x01, 0x02, 0x03, 0x04]:
            logging.warning(f"Invalid mode code received for {light}: {hex(mode_code)}")
            return
        ...
        self.update_mode_indicator(mode_name)
        ...
        if status_code == 0x01:  # ON
            ...
            self.light_status[light]["status"] = 0
            self.led_states[light] = False
            # Don't update mode for FAILED status
            return
        ...
        logging.info(f"Controlled {light}: {status_name}, Mode: {mode_name}")
            ...
            while self.running:
                ...
                if msg:
                    ...
                    if light and len(msg.data) >= 2:
                        ...
                        self.control_light(light, status_code, mode_code)
