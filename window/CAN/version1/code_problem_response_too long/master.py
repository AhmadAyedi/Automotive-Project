...
class CANWindowMaster:
    ...
    def send_can_message(self, window, result, level, level_type, mode):
        try:
            ...
            # We'll use data bytes 0-1 for result and level (same as before)
            # and add level_type and mode in bytes 2-3 (optional for slave implementation)
            msg_data = [result_index, level, LEVEL_TYPES.index(level_type), MODES.index(mode)]
    ...
    def parse_response_frame(self, data):
        """Parse response CAN data into window status with new fields"""
        if len(data) < 8:  # At least 2 bytes per window (result + level)
        ...
        try:
            status = {
                "DR": {
                    ...
                    "level": data[1],
                    "level_type": LEVEL_TYPES[data[2]] if len(data) > 2 else "AUTO",
                    "mode": MODES[data[3]] if len(data) > 3 else "WHONEN"
                },
                "PS": {
                    "result": RESULT_CODES[data[2]],
                    "level": data[3],
                    "level_type": LEVEL_TYPES[data[4]] if len(data) > 4 else "AUTO",
                    "mode": MODES[data[5]] if len(data) > 5 else "WHONEN"
                },
                "DRS": {
                    "result": RESULT_CODES[data[4]],
                    "level": data[5],
                    "level_type": LEVEL_TYPES[data[6]] if len(data) > 6 else "AUTO",
                    "mode": MODES[data[7]] if len(data) > 7 else "WHONEN"
                },
                "PRS": {
                    "result": RESULT_CODES[data[6]],
                    "level": data[7],
                    "level_type": LEVEL_TYPES[data[8]] if len(data) > 8 else "AUTO",
                    "mode": MODES[data[9]] if len(data) > 9 else "WHONEN"
                }
            }
            return status
    ...
    def write_response_to_file(self, status):
        """Write response status to window_response.txt with new format"""
        try:
            with open("window_response.txt", 'a') as f:
                for window in ["DR", "PS", "DRS", "PRS"]:
                    f.write(f"{window} | {status[window]['result']} | {status[window]['level']} | {status[window]['level_type']} | {status[window]['mode']}\n")
                ...
                if msg and msg.arbitration_id == RESPONSE_ID:
                    ...
                    if status:
                        ...
                        for window in status:
                            print(f"{window}: {status[window]['result']} | {status[window]['level']}% | {status[window]['level_type']} | {status[window]['mode']}")
                        ...
                        for line in lines:
                            if line.strip() and line.startswith("Window:"):
                                try:
                                    ...
                                    level_type = parts[3].split(':')[1].strip()
                                    mode = parts[4].split(':')[1].strip()
                                    
                                    if (window in WINDOW_IDS and 0 <= level <= 100 and 
                                        result in RESULT_CODES and 
                                        level_type in LEVEL_TYPES and 
                                        mode in MODES):
