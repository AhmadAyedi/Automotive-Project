...
# Special status codes that require red LED
SPECIAL_STATUS_CODES = ["OP_S", "CL_S", "OPG_S", "CLG_S", "FOP_S"]
...
class CANWindowSlave:
    ...
    def update_status_leds(self, result):
        """Update the status LEDs based on the result code"""
        ...
        elif result in SPECIAL_STATUS_CODES:
            GPIO.output(RED_LED, GPIO.HIGH)
            logging.info(f"Special status detected ({result}), turning on red LED")
        else:
            GPIO.output(GREEN_LED, GPIO.HIGH)
            logging.info(f"Normal status detected ({result}), turning on green LED")
    ...
    def handle_window_message(self, window, result, level, level_type, mode):
        # First update the status LEDs based on the result
        self.update_status_leds(result)
