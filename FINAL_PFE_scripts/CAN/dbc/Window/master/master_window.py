#!/usr/bin/env python3
"""
Professional CAN Window Master Controller - Fixed Version
"""
...
class CANWindowMaster:
    def __init__(self, filename):
        ...
        # Build valid result codes for each window
        self.valid_results = {}
        for window, message in self.message_map.items():
            for signal in message.signals:
                if signal.name.endswith('_RESULT'):
                    self.valid_results[window] = list(signal.choices.values())
                    break
                        ...
                        for line in new_content.split('\n'):
                            if line.strip() and line.startswith("Window:"):
                                try:
                                    ...
                                    if result not in self.valid_results[window]:
                                        print(f"Invalid result for {window}. Valid options: {self.valid_results[window]}")
                                        continue
                                    ...
                                    if level_type not in ["AUTO", "MANUAL"]:
                                        print(f"Invalid level type (AUTO/MANUAL): {level_type}")
                                        continue
                                        
                                    if mode not in ["WHONEN", "FAHREN"]:
                                        print(f"Invalid mode (WHONEN/FAHREN): {mode}")
                                        continue
                                        
                                    if safety not in ["ON", "OFF"]:
                                        print(f"Invalid safety (ON/OFF): {safety}")
                                        continue
