...
class CANLightSlave:
    def __init__(self):
        ...
        # LED Groups using 0-based indexing (from your reference)
        self.LED_GROUPS = {
    ...
    def shift_out(self, data):
        """Shift out data to the 74HC595 shift register (LED 0 to LED 39)"""
        ...
        if status == "ON":
            if light == "LOW_BEAM":
                ...
                if mode in ["FAHREN", "WOHNEN"]:
                    self.turn_on_light("LOW_BEAM")
                    self.light_status[light]["status"] = "ON"
            else:
            ...
            self.light_status[light]["status"] = "OFF"
    ...
    def send_light_response(self, light):
        ...
        try:
            ...
            if status_signal.choices:
                ...
                status_value = 1 if status_data["status"] == "ON" else 0
