...
class LINMaster:
    ...
    def send_message(self):
        """Send 'P' message to slave (1 byte data)"""
        try:
            self.wakeup_slave()
            self.send_break()
            ...
            checksum = self.calculate_checksum(pid, message)
            self.ser.write(bytes([checksum]))
            self.ser.flush()
            
            print(f"Sent LIN frame: PID={hex(pid)}, Data={message}, Checksum={hex(checksum)}")
    ...
    def monitor_responses(self):
        ...
        while self.running:
            if self.ser.in_waiting:
                ...
                # Minimum frame is 5 bytes (header + 1 byte data + checksum)
                if len(buffer) >= 5:
                    ...
                    if frame_id != SLAVE_RESPONSE_ID:
                        print(f"Unexpected response frame ID: {hex(frame_id)}")
